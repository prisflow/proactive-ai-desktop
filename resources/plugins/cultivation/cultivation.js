...
function worldSetting(w) {
  ...
  if (events.length) {
    lines.push(`\u5927\u4E8B\u4EF6\u65F6\u95F4\u7EBF\uFF1A${events.map((e) => `${e.name}\uFF08${e.type}\xB7\u7B2C${e.at}\u6708\u2192\u7B2C${e.by}\u6708\uFF09`).join("\uFF1B")}`);
  }
  return lines.join("\n");
}
...
function makeApplyCharacter(ledger) {
  return (ctx) => {
    ...
    const originRealm = typeof origin?.realm === "string" && (origin.realm === "\u51E1\u4EBA" || origin.realm === "\u7EC3\u6C14") ? origin.realm : "\u51E1\u4EBA";
    s.realm = originRealm;
    s.realmStage = 1;
  ...
  return (ctx) => {
    ...
    if (d.cultivate) {
      ...
      if (method) {
        ...
        const cap2 = cultivationCap(w);
        const before = s.cultivation;
        s.cultivation = Math.min(s.cultivation + rawGain, cap2);
...
function runFlow(api, flowName, input) {
  return api.flow.run(flowName, input).then((res) => {
...
function registerTools(api, ledger, rules) {
  const defs = [
    ...
    },
    {
      ...
      run: (input, meta) => api.flow.run("game_query", input).then((res) => {
